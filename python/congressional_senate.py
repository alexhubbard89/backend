...
speaking_senate = tally_toolkit.Congressional_report_collector()

df = pd.read_sql_query("""
SELECT * FROM congressional_record_senate
;
""", open_connection())
...
for i in df.index:
    if df.loc[i, 'pdf_str'] != 'nan':
        ...
        for section in by_section:
            speaking_senate.text = section
            tally_toolkit.Congressional_report_collector.get_sub_clean_text(speaking_senate)
            clean_df = tally_toolkit.Congressional_report_collector.whatd_they_say(speaking_senate, 'senate')
            if len(clean_df) > 0:
